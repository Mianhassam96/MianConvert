...
import { X, FileVideo, VolumeX, Volume2 } from "lucide-react";
...
const MuteTool = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      if (mode === "mute") {
        // Remove audio track, copy video stream
      ...
      {file && !result && (
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            {([
              { id: "mute" as Mode, icon: VolumeX, label: "Mute Video", desc: "Remove audio, keep video" },
          ...
          <AnimatedButton onClick={handleProcess} loading={processing} className="w-full" size="lg">
            {processing
              ...
              : (mode === "mute" ? "Mute Video" : "Extract Audio as MP3")}
          </AnimatedButton>

          {processing && (
            <AnimatedProgress
              value={progress}
              label={mode === "mute" ? "Removing audio track…" : "Extracting audio…"}
              done={done}
            />
          )}
        </>
